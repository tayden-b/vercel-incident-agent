...
A multi-agent incident-response system I'd actually trust in front of an SRE:
a triage → parallel-diagnosis → resolution pipeline whose every step is
observable, persisted, and gated on human approval. The August 2026 rebuild
replaced the v1 single-shot analyze-and-email loop with real concurrent agents;
the goal now is live data and measured accuracy, not more features.

## Next up

- [ ] Eval harness: score each scenario run against its held-out ground truth
      (diagnosis verdicts + chosen action kind), so prompt/model changes are
      measurable instead of vibes.
- [ ] Screenshots (or a short GIF) of the war room mid-investigation in the
      README — this project demos visually or not at all.

## Later

- [ ] Live mode: ingest real Vercel runtime logs behind the same tool
      interface the scenarios implement (`signature.ts` de-dup carries over).
- [ ] Alert channel for new incidents (the v1 Slack webhook pattern, rebuilt
      against the new pipeline).
- [ ] Multi-project support (poll several Vercel projects, tag incidents).

## Done

- [x] Rebuilt as a multi-agent pipeline (2026-08-19). Triage scopes the
      incident and produces distinct hypotheses; one diagnosis agent per
      hypothesis runs concurrently over read-only tools; a resolution agent
      synthesizes the RCA and proposes actions a human approves in the UI.
      Every reasoning step/tool call streams over SSE and persists to the db.
      Prisma/SQLite → Drizzle/libSQL, raw fetch → AI SDK tool loops. Retired
      with the v1 loop: the Gmail sender, the email approval tokens
      (`approval-token.ts` — approval is now an authenticated UI action, not a
      link), and the v1 Slack alert (returns later, see above). `signature.ts`
      and its tests carried forward for live-mode de-dup.
